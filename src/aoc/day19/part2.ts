...
class BeaconSet {
  ...
  tryMerge(other: BeaconSet): boolean {
    ...
    this.scanners.push(transform({ x: 0, y: 0, z: 0 }));
    return true;
  }
...
const taxicabDistance = (A: Coord, B: Coord): number =>
...
let maxDistance = 0;
let numScanners = base.scanners.length;
for (let i = 0; i < numScanners; i++) {
  for (let j = i + 1; j < numScanners; j++) {
    maxDistance = Math.max(maxDistance, taxicabDistance(base.scanners[i], base.scanners[j]));
  }
}
