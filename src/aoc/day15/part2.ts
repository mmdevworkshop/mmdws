...
const { start, goal } = (() => {
  ...
  // create nodes
  const tileHeight = costGrid.length;

  for (let y = 0; y < tileHeight * 5; y++) {
    const costRow = costGrid[y % tileHeight];
    const tileWidth = costRow.length;

    nodes[y] = [];

    for (let x = 0; x < tileWidth * 5; x++) {
      const tileCost = costRow[x % tileWidth];
      const costModifier = Math.floor(x / tileWidth) + Math.floor(y / tileHeight);
      const cost = ((tileCost + costModifier - 1) % 9) + 1;
  ...
  // bind edges
  const fullHeight = nodes.length;
  for (let y = 0; y < fullHeight; y++) {
    const fullWidth = nodes[y].length;
    for (let x = 0; x < fullHeight; x++) {
